...
contract TestGhoGsmSteward is TestGhoBase, GhoStewardProcedure {
    function setUp() public {
        ...
        GHO_GSM_STEWARD = GhoGsmSteward(
            _deployGhoGsmSteward({
                fixedFeeStrategyFactory: address(FIXED_FEE_STRATEGY_FACTORY), riskCouncil: RISK_COUNCIL
            })
        );
    ...
    function testRevertUpdateGsmExposureCapIfStewardLostConfiguratorRole() public {
        ...
        vm.expectRevert(AccessControlErrorsLib.MISSING_ROLE(GSM_CONFIGURATOR_ROLE, address(GHO_GSM_STEWARD)));
        vm.prank(RISK_COUNCIL);
        GHO_GSM_STEWARD.updateGsmExposureCap(address(GHO_GSM), oldExposureCap + 1);
    }
    ...
    function testUpdateGsmBuySellFeesBothFeesMax() public {
        ...
        uint256 maxFeeUpdate = GHO_GSM_STEWARD.GSM_FEE_RATE_CHANGE_MAX();
        vm.prank(RISK_COUNCIL);
        GHO_GSM_STEWARD.updateGsmBuySellFees(address(GHO_GSM), buyFee + maxFeeUpdate, sellFee + maxFeeUpdate);
        address newStrategy = GHO_GSM.getFeeStrategy();
        uint256 newBuyFee = IGsmFeeStrategy(newStrategy).getBuyFee(1e4);
        uint256 newSellFee = IGsmFeeStrategy(newStrategy).getSellFee(1e4);
        assertEq(newBuyFee, buyFee + maxFeeUpdate);
    ...
    function testUpdateGsmBuySellFeesIfZeroFees() public {
        ...
        vm.mockCall(
            currentFeeStrategy, abi.encodeWithSelector(GHO_GSM_FIXED_FEE_STRATEGY.getBuyFee.selector), abi.encode(0)
        );
        vm.mockCall(
            currentFeeStrategy, abi.encodeWithSelector(GHO_GSM_FIXED_FEE_STRATEGY.getSellFee.selector), abi.encode(0)
        );
    ...
    function testRevertUpdateGsmBuySellFeesIfZeroFeeStrategyAddress() public {
        vm.mockCall(address(GHO_GSM), abi.encodeWithSelector(GHO_GSM.getFeeStrategy.selector), abi.encode(address(0)));
    ...
    function testRevertUpdateGsmBuySellFeesIfBothMoreThanMax() public {
        ...
        vm.prank(RISK_COUNCIL);
        vm.expectRevert("INVALID_BUY_FEE_UPDATE");
        GHO_GSM_STEWARD.updateGsmBuySellFees(address(GHO_GSM), buyFee + maxFeeUpdate + 1, sellFee + maxFeeUpdate + 1);
    }
