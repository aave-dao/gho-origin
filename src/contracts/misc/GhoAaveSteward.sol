...
contract GhoAaveSteward is Ownable, RiskCouncilControlled, IGhoAaveSteward {
    ...
    /// @inheritdoc IGhoAaveSteward
    function updateGhoBorrowCap(uint256 newBorrowCap)
        external
        onlyRiskCouncil
        notTimelocked(_ghoTimelocks.ghoBorrowCapLastUpdate)
    {
        DataTypes.ReserveConfigurationMap memory configuration =
            IPool(IPoolAddressesProvider(POOL_ADDRESSES_PROVIDER).getPool()).getConfiguration(GHO_TOKEN);
        uint256 currentBorrowCap = configuration.getBorrowCap();
        require(newBorrowCap != currentBorrowCap, "NO_CHANGE_IN_BORROW_CAP");
        require(
            _isDifferenceLowerThanMax(currentBorrowCap, newBorrowCap, currentBorrowCap), "INVALID_BORROW_CAP_UPDATE"
        );
    ...
    /// @inheritdoc IGhoAaveSteward
    function updateGhoSupplyCap(uint256 newSupplyCap)
        external
        onlyRiskCouncil
        notTimelocked(_ghoTimelocks.ghoSupplyCapLastUpdate)
    {
        DataTypes.ReserveConfigurationMap memory configuration =
            IPool(IPoolAddressesProvider(POOL_ADDRESSES_PROVIDER).getPool()).getConfiguration(GHO_TOKEN);
        ...
        require(
            _isDifferenceLowerThanMax(currentSupplyCap, newSupplyCap, currentSupplyCap), "INVALID_SUPPLY_CAP_UPDATE"
        );
     ...
     */
    function _validateRatesUpdate(IDefaultInterestRateStrategyV2.InterestRateData memory newRates) internal view {
        address rateStrategyAddress = IPoolDataProvider(POOL_DATA_PROVIDER).getInterestRateStrategyAddress(GHO_TOKEN);
        IDefaultInterestRateStrategyV2.InterestRateData memory currentRates =
            IDefaultInterestRateStrategyV2(rateStrategyAddress).getInterestRateDataBps(GHO_TOKEN);
     ...
     */
    function _isDifferenceLowerThanMax(uint256 from, uint256 to, uint256 max) internal pure returns (bool) {
     ...
     */
    function _updateWithinAllowedRange(uint256 from, uint256 to, uint256 maxPercentChange, bool isChangeRelative)
        internal
        pure
        returns (bool)
    {
