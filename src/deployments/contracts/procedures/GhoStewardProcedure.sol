...
contract GhoStewardProcedure {
    function _deployGhoAaveSteward(
        ...
        return address(
            new GhoAaveSteward(owner, poolAddressesProvider, poolDataProvider, ghoToken, riskCouncil, borrowRateConfig)
        );
    }

    function _deployGhoBucketSteward(address owner, address ghoToken, address riskCouncil) internal returns (address) {
        return address(new GhoBucketSteward(owner, ghoToken, riskCouncil));
    }

    function _deployGhoCcipSteward(address ghoToken, address ghoTokenPool, address riskCouncil, bool bridgeLimitEnabled)
        internal
        returns (address)
    {
    ...
    function _deployGhoGsmSteward(address fixedFeeStrategyFactory, address riskCouncil) internal returns (address) {
