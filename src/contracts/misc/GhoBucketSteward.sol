...
contract GhoBucketSteward is Ownable, RiskCouncilControlled, IGhoBucketSteward {
     ...
     */
    constructor(address owner, address ghoToken, address riskCouncil) RiskCouncilControlled(riskCouncil) {
    ...
    /// @inheritdoc IGhoBucketSteward
    function updateFacilitatorBucketCapacity(address facilitator, uint128 newBucketCapacity)
        external
        onlyRiskCouncil
        notTimelocked(_facilitatorsBucketCapacityTimelocks[facilitator])
    {
    ...
    /// @inheritdoc IGhoBucketSteward
    function setControlledFacilitator(address[] memory facilitatorList, bool approve) external onlyOwner {
     ...
     */
    function _isIncreaseLowerThanMax(uint256 from, uint256 to, uint256 max) internal pure returns (bool) {
