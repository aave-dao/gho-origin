...
contract GhoTokenProcedure {
    ...
    function _getUpgradeableGhoTokenInitializeCalldata(address tokenAdmin) internal pure returns (bytes memory) {
    ...
    function _deployUpgradeableGhoTokenProxy(address implementation, address proxyAdmin, address tokenAdmin)
        internal
        virtual
        returns (address)
    {
        return address(
            new TransparentUpgradeableProxy(
                implementation, proxyAdmin, _getUpgradeableGhoTokenInitializeCalldata(tokenAdmin)
            )
        );
    }
