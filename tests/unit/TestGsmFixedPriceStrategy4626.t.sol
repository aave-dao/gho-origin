...
contract TestGsmFixedPriceStrategy4626 is TestGhoBase {
    function testConstructor(uint256 ratio, address underlying, uint8 decimals) public {
        ...
        assertEq(strategy.UNDERLYING_ASSET_DECIMALS(), decimals, "Unexpected underlying asset decimals");
    }
    ...
    function testTwoToOnePriceRatio() public {
        FixedPriceStrategy4626 strategy = new FixedPriceStrategy4626(0.5e18, address(USDX_4626_TOKEN), 6);
        uint256 usdxIn = 100e6;
    ...
    function testFuzzingExchangeRate(uint256 ratio, uint8 decimals, uint256 amount) public {
        ...
        FixedPriceStrategy4626 strategy = new FixedPriceStrategy4626(ratio, address(USDX_4626_TOKEN), decimals);
        uint256 amountInGho = (amount * ratio) / (10 ** decimals);
        assertEq(strategy.getAssetPriceInGho(amount, false), amountInGho, "Unexpected asset price in GHO");
    }
}
