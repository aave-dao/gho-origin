...
contract TestGelatoOracleSwapFreezer is TestOracleSwapFreezerBase {
    ...
    function _checkAndPerformAutomation(address swapFreezer) internal virtual override returns (bool) {
        (bool shouldRunKeeper, bytes memory encodedPerformCall) = IGelatoOracleSwapFreezer(swapFreezer).checkUpkeep("");
        if (shouldRunKeeper) {
