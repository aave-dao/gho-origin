...
import {
    IFixedFeeStrategyFactory
} from "src/contracts/facilitators/gsm/feeStrategy/interfaces/IFixedFeeStrategyFactory.sol";
...
contract GhoGsmSteward is RiskCouncilControlled, IGhoGsmSteward {
     ...
     */
    constructor(address fixedFeeStrategyFactory, address riskCouncil) RiskCouncilControlled(riskCouncil) {
    ...
    /// @inheritdoc IGhoGsmSteward
    function updateGsmExposureCap(address gsm, uint128 newExposureCap)
        external
        onlyRiskCouncil
        notTimelocked(_gsmTimelocksByAddress[gsm].gsmExposureCapLastUpdated)
    {
    ...
    /// @inheritdoc IGhoGsmSteward
    function updateGsmBuySellFees(address gsm, uint256 buyFee, uint256 sellFee)
        external
        onlyRiskCouncil
        notTimelocked(_gsmTimelocksByAddress[gsm].gsmFeeStrategyLastUpdated)
    {
        ...
        require(_isDifferenceLowerThanMax(currentBuyFee, buyFee, GSM_FEE_RATE_CHANGE_MAX), "INVALID_BUY_FEE_UPDATE");
        require(_isDifferenceLowerThanMax(currentSellFee, sellFee, GSM_FEE_RATE_CHANGE_MAX), "INVALID_SELL_FEE_UPDATE");
