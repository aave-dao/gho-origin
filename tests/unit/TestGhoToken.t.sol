...
contract TestGhoToken is TestGhoBase {
    ...
    function testGetPopulatedFacilitatorsList() public view {
        ...
        assertEq(facilitatorList[0], address(GHO_DIRECT_FACILITATOR), "Unexpected address for mock facilitator 1");
        assertEq(facilitatorList[1], address(GHO_FLASH_MINTER), "Unexpected address for mock facilitator 2");
        assertEq(facilitatorList[2], address(FLASH_BORROWER), "Unexpected address for mock facilitator 3");
    ...
    function testRevertAddExistingFacilitator() public {
        ...
        GHO_TOKEN.addFacilitator(address(GHO_FLASH_MINTER), "FlashMinter Facilitator", DEFAULT_CAPACITY);
    }
    ...
    function testRevertAddFacilitatorNoRole() public {
        vm.expectRevert(AccessControlErrorsLib.MISSING_ROLE(GHO_TOKEN_FACILITATOR_MANAGER_ROLE, address(ALICE)));
        vm.prank(ALICE);
        GHO_TOKEN.addFacilitator(ALICE, "Alice", DEFAULT_CAPACITY);
    }
    ...
    function testRevertSetNewBucketCapacityNoRole() public {
        vm.expectRevert(AccessControlErrorsLib.MISSING_ROLE(GHO_TOKEN_BUCKET_MANAGER_ROLE, address(ALICE)));
        vm.prank(ALICE);
        GHO_TOKEN.setFacilitatorBucketCapacity(address(GHO_FLASH_MINTER), 0);
    }
    ...
    function testRemoveFacilitator() public {
        ...
        assertEq(facilitatorList[0], address(GHO_DIRECT_FACILITATOR), "Unexpected address for facilitator 0");
    }
    ...
    function testBurn() public {
        ...
        emit FacilitatorBucketLevelUpdated(
            address(GHO_FLASH_MINTER), DEFAULT_CAPACITY, DEFAULT_CAPACITY - DEFAULT_BORROW_AMOUNT
        );
    ...
    function testPermitAndVerifyNonce() public {
        (uint8 v, bytes32 r, bytes32 s) = getPermitSignature(david, davidKey, BOB, 1e18, 0, block.timestamp + 1 hours);
    ...
    function testRevertPermitInvalidOwner() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) = getPermitSignature(ALICE, davidKey, BOB, 1e18, 0, block.timestamp + 1 hours);
    ...
    function testRevertPermitInvalidSpender() public {
        assertEq(GHO_TOKEN.allowance(david, BOB), 0, "Unexpected allowance");
        (uint8 v, bytes32 r, bytes32 s) = getPermitSignature(david, davidKey, ALICE, 1e18, 0, block.timestamp + 1 hours);
        vm.expectRevert(bytes("INVALID_SIGNER"));
        GHO_TOKEN.permit(david, BOB, 1e18, block.timestamp + 1 hours, v, r, s);
        assertEq(GHO_TOKEN.allowance(david, BOB), 0, "Unexpected allowance");
    }
    ...
    function testPermitWithMaximumDeadline() public {
        (uint8 v, bytes32 r, bytes32 s) = getPermitSignature(david, davidKey, BOB, 1e18, 0, type(uint256).max);
    ...
    function testPermitCancel() public {
        (uint8 v, bytes32 r, bytes32 s) = getPermitSignature(david, davidKey, BOB, 1e18, 0, block.timestamp + 1);
        GHO_TOKEN.permit(david, BOB, 1e18, block.timestamp + 1, v, r, s);
    ...
    function testRevertPermitInvalidNonce() public {
        (uint8 v, bytes32 r, bytes32 s) = getPermitSignature(david, davidKey, BOB, 1e18, 1, block.timestamp + 1);
        vm.expectRevert(bytes("INVALID_SIGNER"));
        GHO_TOKEN.permit(david, BOB, 1e18, block.timestamp + 1, v, r, s);
    }
