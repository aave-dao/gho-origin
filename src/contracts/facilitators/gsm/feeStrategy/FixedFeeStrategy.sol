...
contract FixedFeeStrategy is IGsmFeeStrategy {
    ...
    function getGrossAmountFromTotalBought(uint256 totalAmount) external view returns (uint256) {
        if (totalAmount == 0) {
            ...
            return totalAmount.mulDiv(
                PercentageMath.PERCENTAGE_FACTOR, PercentageMath.PERCENTAGE_FACTOR + _buyFee, Math.Rounding.Down
            );
        }
    }
    ...
    function getGrossAmountFromTotalSold(uint256 totalAmount) external view returns (uint256) {
        if (totalAmount == 0) {
            ...
            return totalAmount.mulDiv(
                PercentageMath.PERCENTAGE_FACTOR, PercentageMath.PERCENTAGE_FACTOR - _sellFee, Math.Rounding.Up
            );
        }
    }
}
