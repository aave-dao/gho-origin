...
abstract contract TestUpgradeableGhoTokenSetup is TestGhoBase {
    ...
    function setUp() public virtual {
        (address ghoTokenProxy, address ghoTokenImpl) = _deployGhoTokenProxy(PROXY_ADMIN_OWNER, address(this));
        ghoTokenImplementation = ghoTokenImpl;
        ghoToken = UpgradeableGhoToken(ghoTokenProxy);
    }

    function _deployGhoTokenProxy(address proxyAdmin, address tokenAdmin)
        internal
        virtual
        returns (address ghoTokenProxy, address ghoTokenImpl)
    {}
...
abstract contract TestUpgradeableGhoTokenBase is TestUpgradeableGhoTokenSetup {
    ...
    function testInit() public {
        ...
        bytes memory ghoTokenImpleParams = abi.encodeCall(UpgradeableGhoToken.initialize, (address(this)));
        ...
        TransparentUpgradeableProxy ghoTokenProxy =
            new TransparentUpgradeableProxy(address(ghoTokenImple), PROXY_ADMIN_OWNER, ghoTokenImpleParams);
    ...
    function testGetPopulatedFacilitatorsList() public view {
        ...
        assertEq(facilitatorList[0], address(GHO_FLASH_MINTER), "Unexpected address for mock facilitator 1");
    ...
    function testPermitAndVerifyNonce() public {
        ...
        bytes32 outerHash = keccak256(abi.encodePacked("\x19\x01", ghoToken.DOMAIN_SEPARATOR(), innerHash));
...
abstract contract TestUpgradeableGhoTokenUpgradeBase is TestUpgradeableGhoTokenSetup {
    function testInitialization() public {
        ...
        // Implementation
        assertEq(
            ghoTokenImplementation, getProxyImplementationAddress(address(ghoToken)), "proxy implementation is wrong"
        );
        assertEq(UpgradeableGhoToken(ghoTokenImplementation).decimals(), 18, "Wrong default ERC20 decimals");
        vm.expectRevert("Initializable: contract is already initialized");
    ...
    function testUpgrade() public {
        ...
        TransparentUpgradeableProxy(payable(address(ghoToken))).upgradeToAndCall(address(newImpl), mockImpleParams);
