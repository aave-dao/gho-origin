...
abstract contract OracleSwapFreezerBase {
    ...
    constructor(
        ...
        require(
            _validateBounds(freezeLowerBound, freezeUpperBound, unfreezeLowerBound, unfreezeUpperBound, allowUnfreeze),
            "BOUNDS_NOT_VALID"
        );
        GSM = gsm;
    ...
    function _isActionAllowed(Action actionToExecute) internal view returns (bool) {
        uint256 oraclePrice = IPriceOracle(ADDRESS_PROVIDER.getPriceOracle()).getAssetPrice(UNDERLYING_ASSET);
    ...
    function _validateBounds(
        ...
        if (freezeLowerBound >= freezeUpperBound) {
            ...
            if (
                unfreezeLowerBound >= unfreezeUpperBound || freezeLowerBound >= unfreezeLowerBound
