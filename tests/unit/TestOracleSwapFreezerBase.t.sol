...
abstract contract TestOracleSwapFreezerBase is TestGhoBase {
    ...
    function testCheckUpkeepCanFreeze() public {
        ...
        assertGt(type(uint128).max, DEFAULT_FREEZE_UPPER_BOUND, "uint128.max not greater than freeze upper bound");
    ...
    function testCheckUpkeepCanUnfreeze() public {
        ...
        PRICE_ORACLE.setAssetPrice(
            address(USDX_TOKEN), (DEFAULT_UNFREEZE_LOWER_BOUND + DEFAULT_UNFREEZE_UPPER_BOUND) / 2
        );
        canPerformUpkeep = _checkAutomation(swapFreezer);
    ...
    function testGetCanUnfreeze() public {
        assertEq(OracleSwapFreezerBase(swapFreezer).getCanUnfreeze(), true, "Unexpected initial unfreeze state");
        swapFreezer = _deployOracleSwapFreezer(
            GHO_GSM,
            address(USDX_TOKEN),
            IPoolAddressesProvider(address(PROVIDER)),
        ...
        assertEq(OracleSwapFreezerBase(swapFreezer).getCanUnfreeze(), false, "Unexpected final unfreeze state");
    }
