...
contract sGhoPrecisionTest is TestnetProcedures {
    ...
    function test_rate_per_second_actual_values() public {
        ...
        for (uint256 i = 0; i < rates.length; i++) {
            ...
            emit log_named_uint("Difference", contractRatePerSecond > exactRatePerSecond ? 
                contractRatePerSecond - exactRatePerSecond : exactRatePerSecond - contractRatePerSecond);
            emit log("---");
        }
    }
    ...
    function test_linear_growth_factors_for_python() public {
        ...
        for (uint256 r = 0; r < rates.length; r++) {
            for (uint256 p = 0; p < periods.length; p++) {
                ...
                string memory periodDesc = periods[p] == 1 ? "1s" : 
                                         periods[p] == 60 ? "1m" : 
                                         periods[p] == 3600 ? "1h" : 
                                         periods[p] == 86400 ? "1d" : 
                                         periods[p] == 604800 ? "1w" : "1M";
    ...
    function calculateExpectedGrowthFactor(uint16 rateBps, uint256 timeSeconds) internal pure returns (uint256) {
    ...
    function calculatePrecisionLoss(uint256 actual, uint256 expected) internal pure returns (uint256) {
