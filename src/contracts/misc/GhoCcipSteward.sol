...
contract GhoCcipSteward is RiskCouncilControlled, IGhoCcipSteward {
    ...
    /// @inheritdoc IGhoCcipSteward
    function updateBridgeLimit(uint256 newBridgeLimit)
        external
        onlyRiskCouncil
        notTimelocked(_ccipTimelocks.bridgeLimitLastUpdate)
    {
    ...
    function updateRateLimit(
        ...
        RateLimiter.TokenBucket memory inboundConfig =
            IUpgradeableLockReleaseTokenPool(GHO_TOKEN_POOL).getCurrentInboundRateLimiterState(remoteChainSelector);

        require(
            outboundEnabled != outboundConfig.isEnabled || outboundCapacity != outboundConfig.capacity
                || outboundRate != outboundConfig.rate || inboundEnabled != inboundConfig.isEnabled
                || inboundCapacity != inboundConfig.capacity || inboundRate != inboundConfig.rate,
        ...
        require(
            _isDifferenceLowerThanMax(inboundConfig.rate, inboundRate, inboundConfig.rate), "INVALID_RATE_LIMIT_UPDATE"
        );
        ...
        IUpgradeableLockReleaseTokenPool(GHO_TOKEN_POOL)
            .setChainRateLimiterConfig(
                remoteChainSelector,
                RateLimiter.Config({isEnabled: outboundEnabled, capacity: outboundCapacity, rate: outboundRate}),
